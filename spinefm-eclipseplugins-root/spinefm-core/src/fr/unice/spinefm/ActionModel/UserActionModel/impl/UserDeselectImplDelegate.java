...
public class UserDeselectImplDelegate extends UserDeselectImpl {
	
	protected static Logger log = Logger.getLogger(UserDeselectImplDelegate.class);
	
	
	@Override
	public void apply() throws ElementNotFoundException, FatalSpineFMException {
		log.info("\n\n\t\t Apply user action Deselect on feature "+this.getFeatureName()+" on concept "+this.getDomainElementName()+" and context "+this.getContextID()+"\n\n");
		...
		ActionDeselect as = SystemActionModelFactory.eINSTANCE.createActionDeselect();
	...
	public String getDescription() {
		return "Deselect feature "+this.getFeatureName()+" in concept "+this.getDomainElementName();
	}
	
	@Override
	public UserAction cloneActionWithStringAttributes() {
		UserDeselect result = UserActionModelFactory.eINSTANCE.createUserDeselect();
